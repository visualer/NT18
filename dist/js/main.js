...
function scroll_main(width, number) {
    ...
    $(".slider").css({"width": width, "height": (width * 576 / 1024)});

    s.css({"width": width * (number + 1), "left": -width});
    call = setInterval(scroll, 5000);
...
function loadData(delay, id) {
    ...
    var $article = $("article"), $id = $("#" + id), $prevDiv = $("#prevDiv");
    if ($id.length === 0) {
...
function panorama(divId, width, posID, povList, errDivId) {
    ...
    if (isPC()) {
        require(['async!BMap'], function () {
            ...
            $divId.css({"text-align": "center", "font-size": "20px"});
...
var x = document.getElementsByClassName("subNav"), i, sum = 0;
...
$("nav").css({"margin-left": (globalWidth - sum) / 2, "width": sum});
$("aside").css("margin-left", (globalWidth - 1060) / 2);
var n = $("#newsBlock");
n.find("ul li a").css("width",
    (n.find("ul").height() > 98) ?
        ((navigator.userAgent.indexOf("Chrome") > -1) ? 240 : 260 - 10 - scrollbarWidth) : 250);
$("body").css("width", globalWidth);
var k = $("article");
k.css("left", (globalWidth - 540) / 2);
...
var $maskLayer = $("#maskLayer"), $img = $maskLayer.find("img");

$(function () {
    $maskLayer.delay(1000);
    $img.delay(1000);
    $img.animate({"left": "+=1%", "top": "+=2%", "opacity": "0"}, 600);
    $maskLayer.animate({"opacity": "0"}, 600, function () {$maskLayer.css("display", "none");});
});
